...
const LargeDevicesInfoContainer = ({ ticketPrice, date, attendingCount }) => {
    return (
        <div className="bg-gray-50 relative mx-16 w-96 col-span-3 h-52 hidden flex-col lg:flex border-2 rounded-xl border-black shadow-2xl items-center justify-center">
            ...
            <div className="flex flex-col items-center justify-center">
                <button className="w-80 mt-3 mb-2 bg-pink-700 rounded-lg font-semibold text-white text-lg px-1 py-3 hover:bg-pink-800 ease-in-out duration-150">Attend</button>
                <div className="font-semibold text-lg m-3">{attendingCount === 0 ? 'Be the first one to attend!' : `Currently ${attendingCount} attending!`}</div>
            </div>
        </div>
    )
}
