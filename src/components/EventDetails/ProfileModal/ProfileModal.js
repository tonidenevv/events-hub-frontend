        ...
        <div id="backdrop" onClick={handleCloseProfileModal} className="fixed z-50 inset-0 flex items-center justify-center bg-opacity-30 backdrop-blur-sm">
            {isLoading ? <Spinner /> :
                <div className="bg-slate-100 lg:w-[36rem] w-80 h-[40rem] lg:h-[42rem] border-2 flex flex-col items-center relative border-black rounded-lg">
                    ...
                    {user.createdEvents.length === 0 ? <div className="font-semibold text-center text-xl h-36 lg:h-48 lg:text-2xl mt-8">No Created Events...</div> :
                    ...
                    {user.attending.length === 0 ? <div className="font-semibold text-center text-xl h-36 lg:h-48 lg:text-2xl mt-8">Not Attending To Any Events...</div> :
