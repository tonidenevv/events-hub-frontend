    ...
    return (
        <Link to={`/events/${event._id}`}>
            <div className="flex flex-col items-center justify-center relative rounded-2xl">
                <div onMouseOver={() => handleHover(true)} onMouseOut={() => handleHover(false)} className="absolute shadow-2xl rounded-2xl ease-in-out duration-100 cursor-pointer w-9/12 h-[22rem] hover:bg-transparent/40 hover:w-10/12 hover:h-[23rem]  bg-black/70 text-white">
                    <p className="font-bold text-2xl px-3 py-10">{event.title}</p>
                    <p className="font-bold text-xl absolute bottom-1 left-3">{format(event.eventDate, "dd/MM/yyyy")}</p>
                    ...
                    <div className="absolute bottom-8 left-3 text-xl">{daysDifference(event.eventDate)}</div>
                </div>
